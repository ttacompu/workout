...
angular.module('workoutFeature').value("appEvents", {workout: { exerciseStarted: "event:workout:exerciseStarted" }});

angular.module('workoutFeature').provider("workoutService", function () {
    ...
    this.$get = ['WorkoutPlan', 'Exercise', '$http', '$q',"$resource", function (WorkoutPlan, Exercise, $http, $q, $resource) {
        ...
        service.getExercise = function (name){
            return service.Exercises.query().$promise.then(function(response){
                var result;
        ...
        service.updateExercise = function(exercise){
            return service.getExercise(exercise.name)
                .then(function(original){
                    if(original){
                        var exerciseToUpdate = angular.copy(exercise);
                        return $http.put(collectionsUrl + "/exercises/" + original.name, exerciseToUpdate);
                    }
        ...
        service.addExercise = function(exercise){
            if(exercise.name){
                var exerciseToSave = angular.copy(exercise);
                exerciseToSave._id = exerciseToSave.name;
                return $http.post(collectionsUrl + "/exercises", exerciseToSave).then(function(response){
                        return exercise;
                });
            }
        }
